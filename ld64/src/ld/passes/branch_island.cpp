...
if (_s_log && (abs(displacement) > kBetweenRegions) ) fprintf(stderr, "from %s to %s delta : 0x%0" PRIx64 " in section %s\n",
				...
				if ( crossSectionBranch && preload && ((displacement > kBranchLimit) || (displacement < (-kBranchLimit))) ) {
					...
					if (_s_log) fprintf(stderr, "  +need forward branching island srcAdr=0x%08"PRIx64", dstAdr=0x%08"PRIx64", target=%s regions count %d\n",
														srcAddr, dstAddr, target->name(), kIslandRegionsCount);
					for (int i=kIslandRegionsCount-1; i >=0 ; --i) {
						AtomToIsland* region = regionsMap[i];
						int64_t islandRegionAddr = regionAddresses[i];
					    if (_s_log) fprintf(stderr, "  island [%d] islandRegionAddr=0x%08"PRIx64"\n", i, islandRegionAddr);
...
fprintf(stderr, "**Want to insert branch island into non-code section %s/%s, wanted this address=0x%08" PRIu64 "\n",
	...
	for(int i=0; i < kIslandRegionsCount; ++i) {
		...
		regionAddresses[i] = sect->address + branchIslandInsertionPoints[i]->sectionOffset() + branchIslandInsertionPoints[i]->size();
		if (_s_log) fprintf(stderr, "ld: branch islands will be inserted at 0x%08"PRIx64" after %s in section %s/%s\n",
...
{	
	...
	// only ARM and PPC need branch islands
	switch ( opts.architecture() ) {
		case CPU_TYPE_ARM:
		case CPU_TYPE_POWERPC:
		case CPU_TYPE_POWERPC64:
