...
template <>
uint64_t LocalRelocationsAtom<ppc64>::relocBaseAddress(ld::Internal& state)
{
  if ( _options.outputKind() != Options::kDynamicExecutable ) {
		// for things other than a final exec. don't mess with it.
		return _options.baseAddress();
  }

  // FIXME: some other tests here to give warnings for 10.4 etc.
  // for all other kinds, the ppc64 reloc base address starts at __DATA segment
  for (std::vector<ld::Internal::FinalSection*>::iterator sit = state.sections.begin(); sit != state.sections.end(); ++sit) {
		ld::Internal::FinalSection* sect = *sit;
...
template <>
uint64_t ExternalRelocationsAtom<ppc64>::relocBaseAddress(ld::Internal& state)
{
	// FIXME: for ppc64, it's more complicated some extra tests needed here to warn of
	// out-of-rage conditions for 10.4
	// ... but, mostly, reloc base address starts at __DATA segment
	for (std::vector<ld::Internal::FinalSection*>::iterator sit = state.sections.begin(); sit != state.sections.end(); ++sit) {
		ld::Internal::FinalSection* sect = *sit;
		if ( strcmp(sect->segmentName(), "__DATA") == 0 )
			return sect->address;
